    ...
    for stock_ticker in stocks:  # purchases stocks based on daytrading patterns
        try:
            ...
            if stock_score >= 0.2 and stock_ticker not in all_active_positions.keys():
                ...
                alpaca.create_order(stock_ticker, 50)
...
def news_stock_analyzer(stock_ticker):
    try:
        ...
        if stock_score >= 0.25 and stock_ticker not in all_active_positions.keys():
            # todo: calculate order amount
            alpaca.create_order(stock_ticker, 50)
            active_positions_to_check[stock_ticker] = sdg.get_current_stock_data(stock_ticker)[
                'Close']
...
def check_perform_sell(stock_ticker, purchase_price):
    while True:
        ...
        print("Checking", stock_ticker, "Gains/Losses",
              price_change_percent, "Price: $", current_stock_price)
        ...
        try:
            ...
            if current_time > const.STOCK_MARKET_OPEN_TIME and current_time < const.STOCK_MARKET_CLOSE_TIME:
                ...
                for partition in partitioned_stocks:
                    threading.Thread(target=daytrading_stock_analyzer, args=[
                                     partition]).start()
            else:
                alpaca.sell_all_positions()
                print("Market Close")
                for stock_ticker in const.STOCKS_TO_CHECK:  # purchases stocks based on news info
                    threading.Thread(target=news_stock_analyzer,
                                     args=(stock_ticker,)).start()
                time.sleep(360000)
        except Exception as e:
            print("Restarting")
